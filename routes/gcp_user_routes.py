...
gcp_user_router = APIRouter()


# Route for index.html
@gcp_user_router.get(path="/gcp_user/v1/userinfo",
...
@gcp_user_router.get("/gcp_user/v1/emails", tags=["Google's User"])
async def read_gmail_emails(request: Request, user_info: dict = Depends(get_current_user)):
    ...
    for thread in threads:
        ...
        thread_detail_response = requests.get(thread_detail_url, headers=headers)
